...
import wan
from wan.utils.utils import cache_video, cache_image, str2bool, cache_single_video
...
from enhance_a_video.globals import enable_enhance, set_enhance_weight
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="Context guidance scale.")
    parser.add_argument(
        "--uncond_type",
        type=str,
        default="text_context_separate",
        choices=["text", "text_context", "text_context_separate"])
    parser.add_argument(
        "--device_id",
        type=int,
        default=1,
        help="CUDA device ID to run the model on (default: 1)."
    )
    parser.add_argument(
        "--context_scale",
        type=float,
        default=1.0,
        help="The context scale for the model.")
    parser.add_argument(
        "--teacache_thresh",
        type=float,
        default=0.2,
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="The weight of enhance, default 4.0")  
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="consistency",
        help="Adapter name. default consistency")
    parser.add_argument(
        "--skip",
        type=int,
        default=0,
        help="How many timesteps to skip at the beginning."
    )
    return parser
...
def main(args):
    ...
    if args.pretrained_lora_path is not None and args.pretrained_lora_path != '':
        wan_vace.add_lora_to_vace(pretrained_lora_path=args.pretrained_lora_path, lora_rank=args.lora_rank, lora_alpha=args.lora_alpha,
                                    lora_target_modules="vace_blocks\\.\\d+\\.(?:self_attn)\\.(?:q|k|v)$",
    ...
    T = src_video[0].shape[1]
    if T < args.frame_num:
        frame_indices = torch.linspace(0, T - 1, args.frame_num).round().long()
        src_video[0] = src_video[0][:, frame_indices]
        src_mask[0] = src_mask[0][:, frame_indices]

    logging.info(f"Generating video...")
    if args.enable_teacache:
        wan_vace.model.teacache_init(
    ...
    if args.enhance:
        enable_enhance()
        set_enhance_weight(args.enhance_weight)
    ...
    video = wan_vace.generate(
        ...
        uncond_type=args.uncond_type,
        seed=args.base_seed,
        offload_model=args.offload_model,
        context_scale=args.context_scale if hasattr(args, 'context_scale') else 1.0,
        skip=args.skip
    )
    ...
    if rank == 0:
        ...
        cache_video(
            tensor=torch.stack([src_video[0], video], dim=0),
            save_file=save_file,
            fps=cfg.sample_fps,
            nrow=1,
            normalize=True,
            value_range=(-1, 1))
        if args.save_separate_vid:
            original_save_file = save_file.replace('.mp4', '_styled.mp4')
            cache_single_video(
                ...
                normalize=True,
                value_range=(-1, 1))
            original_save_file = save_file.replace('.mp4', '_ref.mp4')
            cache_single_video(
                tensor=src_video[0].unsqueeze(0),
                save_file=original_save_file,
                fps=cfg.sample_fps,
                normalize=True,
                value_range=(-1, 1))
